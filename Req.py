            ...
            try:
                ...
                while len(tweets) < count and len(cursor) > 1:
                    response = requests.get(
                        self.new_url(query, cursor), headers=self.HEADERS, cookies=self.COOKIES
                    )
                    if response.status_code == 200:
                        instr = response.json().get("data").get("search_by_raw_query").get("search_timeline").get("timeline")
                        items = (
                            instr
                            .get("instructions")[0]
                            .get("entries")
                        ...
                        if rez_parser == 1:
                            cursor = instr.get("instructions")[2].get("entry").get('content').get('value')
